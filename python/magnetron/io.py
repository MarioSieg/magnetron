...
class StorageArchive:
    ...
    def __enter__(self) -> StorageArchive:
        self._ptr = _C.mag_storage_open(self._ctx.native_ptr, self._path.encode('utf-8'), bytes(self._mode, 'utf-8'))
    ...
    def __exit__(self, exc_type: type[BaseException] | None, exc_value: BaseException | None, traceback: TracebackType | None) -> None:
        if self._ptr != _FFI.NULL:
            if not _C.mag_storage_close(self._ptr):
                raise RuntimeError(f'Failed to close storage archive at {self._path}')
            self._ptr = _FFI.NULL

    def tensor_keys(self) -> list[str]:
        count = _FFI.new('size_t[1]')
        keys_ptr = _C.mag_storage_get_all_tensor_keys(self._ptr, count)
        keys = [_FFI.string(keys_ptr[i]).decode('utf-8') for i in range(count[0])]
        _C.mag_storage_get_all_keys_free(keys_ptr, count[0])
        return keys

    def metadata_keys(self) -> list[str]:
        count = _FFI.new('size_t[1]')
        keys_ptr = _C.mag_storage_get_all_metadata_keys(self._ptr, count)
        keys = [_FFI.string(keys_ptr[i]).decode('utf-8') for i in range(count[0])]
        _C.mag_storage_get_all_keys_free(keys_ptr, count[0])
        return keys

    def get_tensor(self, key: str) -> Tensor | None:
        ptr = _C.mag_storage_get_tensor(self._ptr, key.encode('utf-8'))
        if ptr == _FFI.NULL:
            return None
        return Tensor(ptr)
    ...
    def set_tensor(self, key: str, tensor: Tensor) -> bool:
        ...
        return _C.mag_storage_set_tensor(self._ptr, key.encode('utf-8'), tensor.native_ptr)

    def get_metadata(self, key: str) -> int | float | None:
        data_type = _C.mag_storage_get_metadata_type(self._ptr, key.encode('utf-8'))
        if data_type == _C.MAG_RECORD_TYPE_I64:
            value = _FFI.new('int64_t[1]')
            if not _C.mag_storage_get_metadata_i64(self._ptr, key.encode('utf-8'), value):
                return None
            return int(value[0])
        elif data_type == _C.MAG_RECORD_TYPE_F64:
            value = _FFI.new('double[1]')
            if not _C.mag_storage_get_metadata_f64(self._ptr, key.encode('utf-8'), value):
                return None
    ...
    def set_metadata(self, key: str, metadata: int | float) -> bool:
        if isinstance(metadata, int):
            return _C.mag_storage_set_metadata_i64(self._ptr, key.encode('utf-8'), metadata)
        elif isinstance(metadata, float):
            return _C.mag_storage_set_metadata_f64(self._ptr, key.encode('utf-8'), metadata)
        else:
            return False
