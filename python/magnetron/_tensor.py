...
class Tensor:
    ...
    def fill_random_uniform_(self, from_: float | int | None = None, to: float | int | None = None) -> None:
        assert self.dtype.is_numeric, f'Requires a numeric dtype: {NUMERIC_DTYPES}'
        self._validate_inplace_op()
    ...
    def fill_random_normal_(self, mean: float, std: float) -> None:
        assert self.dtype.is_floating_point, f'Requires floating-point dtype: {FLOATING_POINT_DTYPES}'
        self._validate_inplace_op()
        _C.mag_tensor_fill_random_normal(self._ptr, mean, std)

    def fill_random_bernoulli_(self, p: float) -> None:
        assert self.dtype == boolean, f'Requires boolean dtype: {boolean}'
        self._validate_inplace_op()
    ...
    def logical_and(self, other: Tensor) -> Tensor:
        assert self.dtype.is_integral, f'First arg requires integral dtype: {INTEGRAL_DTYPES}'
        assert other.dtype.is_integral, f'Second arg requires integral dtype: {INTEGRAL_DTYPES}'
    ...
    def logical_not(self) -> Tensor:
        assert self.dtype.is_integral, f'Requires integral dtype: {INTEGRAL_DTYPES}'
    ...
    def __add__(self, other: Tensor | int | float) -> Tensor:
        if not isinstance(other, Tensor):
            other = Tensor.full(self.shape, dtype=self.dtype, fill_value=float(other))
        assert self.dtype.is_numeric, f'First arg requires numeric dtype: {NUMERIC_DTYPES}'
        assert other.dtype.is_numeric, f'Second arg requires numeric dtype: {NUMERIC_DTYPES}'
    ...
    def __iadd__(self, other: Tensor | int | float) -> Tensor:
        self._validate_inplace_op()
        if not isinstance(other, Tensor):
            val: float | int = float(other) if self.dtype.is_floating_point else int(other)
    ...
    def __ifloordiv__(self, other: Tensor | int | float) -> Tensor:
        self._validate_inplace_op()
        if not isinstance(other, Tensor):
            val: float | int = float(other) if self.dtype.is_floating_point else int(other)
            other = Tensor.full(self.shape, dtype=self.dtype, fill_value=val)
    ...
    def __matmul__(self, other: Tensor) -> Tensor:
        assert self.dtype.is_floating_point, f'First arg requires floating-point dtype: {FLOATING_POINT_DTYPES}'
        assert other.dtype.is_floating_point, f'Second arg requires floating-point dtype: {FLOATING_POINT_DTYPES}'
    ...
    def __and__(self, other: Tensor | bool | int) -> Tensor:
        if not isinstance(other, Tensor):
    ...
    def __iand__(self, other: Tensor | int | float) -> Tensor:
        self._validate_inplace_op()
        if not isinstance(other, Tensor):
            val: bool | int = bool(other) if self.dtype == boolean else int(other)
            other = Tensor.full(self.shape, dtype=self.dtype, fill_value=val)
    ...
    def __or__(self, other: Tensor | bool | int) -> Tensor:
        if not isinstance(other, Tensor):
            val: bool | int = bool(other) if self.dtype == boolean else int(other)
            other = Tensor.full(self.shape, dtype=self.dtype, fill_value=val)
        assert self.dtype.is_integral, f'First arg requires integral dtype: {INTEGRAL_DTYPES}'
    ...
    def __ior__(self, other: Tensor | int | float) -> Tensor:
        self._validate_inplace_op()
        if not isinstance(other, Tensor):
            val: bool | int = bool(other) if self.dtype == boolean else int(other)
            other = Tensor.full(self.shape, dtype=self.dtype, fill_value=val)
    ...
    def __xor__(self, other: Tensor | bool | int) -> Tensor:
        if not isinstance(other, Tensor):
            val: bool | int = bool(other) if self.dtype == boolean else int(other)
            other = Tensor.full(self.shape, dtype=self.dtype, fill_value=val)
        assert self.dtype.is_integral, f'First arg requires integral dtype: {INTEGRAL_DTYPES}'
    ...
    def __ixor__(self, other: Tensor | int | float) -> Tensor:
        self._validate_inplace_op()
        if not isinstance(other, Tensor):
            val: bool | int = bool(other) if self.dtype == boolean else int(other)
            other = Tensor.full(self.shape, dtype=self.dtype, fill_value=val)
    ...
    def __invert__(self) -> Tensor:
        assert self.dtype.is_integral, f'Requires integral dtype: {FLOATING_POINT_DTYPES}'
        return Tensor(_C.mag_not(self._ptr))

    def __lshift__(self, other: Tensor | int) -> Tensor:
        if not isinstance(other, Tensor):
            other = Tensor.full(self.shape, dtype=self.dtype, fill_value=int(other))
        assert self.dtype.is_integer, f'First arg requires integer dtype: {INTEGER_DTYPES}'
        assert other.dtype.is_integer, f'Second arg requires integer dtype: {INTEGER_DTYPES}'
    ...
    def __ilshift__(self, other: Tensor | int) -> Tensor:
        self._validate_inplace_op()
        if not isinstance(other, Tensor):
            other = Tensor.full(self.shape, dtype=self.dtype, fill_value=int(other))
        assert self.dtype.is_integer, f'First arg requires integer dtype: {INTEGER_DTYPES}'
    ...
    def __irshift__(self, other: Tensor | int) -> Tensor:
        self._validate_inplace_op()
        if not isinstance(other, Tensor):
            other = Tensor.full(self.shape, dtype=self.dtype, fill_value=int(other))
        assert self.dtype.is_integer, f'First arg requires integer dtype: {INTEGER_DTYPES}'
    ...
    def __eq__(self, other: Tensor | list[int | float | bool] | int | float | bool) -> Tensor:
        if isinstance(other, list):
            other = Tensor.of(other, dtype=self.dtype)
        elif not isinstance(other, Tensor):
            other = Tensor.full(self.shape, dtype=self.dtype, fill_value=other)
        return Tensor(_C.mag_eq(self._ptr, other._ptr))

    def __ne__(self, other: Tensor | list[int | float | bool] | int | float | bool) -> Tensor:
        if isinstance(other, list):
            other = Tensor.of(other, dtype=self.dtype)
        elif not isinstance(other, Tensor):
            other = Tensor.full(self.shape, dtype=self.dtype, fill_value=other)
