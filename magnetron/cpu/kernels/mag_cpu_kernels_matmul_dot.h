...
typedef void (mag_dot_fp8w_scaled_kernel_contig_t)(int64_t numel, void *r, const void *px, const void *py, float scale);
#define mag_dot_fp8w_scaled_kernel_contig_impl(T, TtoF32, F32toT) \
  static MAG_HOTPROC void mag_dot_fp8w_scaled_kernel_contig_##T(int64_t numel, void *pr, const void *px, const void *py, float scale) { \
    const T *x = (const T *)px; \
    const mag_float8_e4m3fn_t *y = (const mag_float8_e4m3fn_t *)py; \
    float acc=0.f; \
    for (int64_t i=0; i < numel; ++i) \
      acc += TtoF32(x[i])*mag_float8_e4m3fn_to_float32(y[i]); \
    *(T *)pr = F32toT(acc*scale); \
  }
...
mag_dot_fp8w_scaled_kernel_contig_impl(mag_bfloat16_t, mag_bfloat16_to_float32, mag_float32_to_bfloat16)
mag_dot_fp8w_scaled_kernel_contig_impl(mag_float8_e4m3fn_t, mag_float8_e4m3fn_to_float32, mag_float32_to_float8_e4m3fn)
#undef mag_dot_fp8w_scaled_kernel_contig_impl

typedef void (mag_dot_fp8w_scaled_kernel_strided_t)(int64_t numel, void *r, const void *px, const void *py, int64_t sx, int64_t sy, float scale);
#define mag_dot_fp8w_scaled_kernel_strided_impl(T, TtoF32, F32toT) \
  static MAG_HOTPROC void mag_dot_fp8w_scaled_kernel_strided_##T(int64_t numel, void *pr, const void *px, const void *py, int64_t sx, int64_t sy, float scale) { \
    const T *x = (const T *)px; \
    const mag_float8_e4m3fn_t *y = (const mag_float8_e4m3fn_t *)py; \
    float acc=0.f; \
    for (int64_t i=0; i < numel; ++i) \
      acc += TtoF32(x[i*sx])*mag_float8_e4m3fn_to_float32(y[i*sy]); \
    *(T *)pr = F32toT(acc*scale); \
  }
...
mag_dot_fp8w_scaled_kernel_strided_impl(mag_bfloat16_t, mag_bfloat16_to_float32, mag_float32_to_bfloat16)
mag_dot_fp8w_scaled_kernel_strided_impl(mag_float8_e4m3fn_t, mag_float8_e4m3fn_to_float32, mag_float32_to_float8_e4m3fn)
#undef mag_dot_fp8w_scaled_kernel_strided_impl

static MAG_HOTPROC void mag_matmul_dot_fp8w_scaled_impl(
  mag_dtype_t dtype,
  int64_t ti,
  int64_t N,
  void *pr,
  ...
  if (ti != 0) return;
  static mag_dot_fp8w_scaled_kernel_contig_t *const kernel_lut_contig[4] = {
    [MAG_DTYPE_FLOAT32] = &mag_dot_fp8w_scaled_kernel_contig_float,
    [MAG_DTYPE_FLOAT16] = &mag_dot_fp8w_scaled_kernel_contig_mag_float16_t,
    [MAG_DTYPE_BFLOAT16] = &mag_dot_fp8w_scaled_kernel_contig_mag_bfloat16_t,
    [MAG_DTYPE_FLOAT8_E4M3FN] = &mag_dot_fp8w_scaled_kernel_contig_mag_float8_e4m3fn_t
  };
  static mag_dot_fp8w_scaled_kernel_strided_t *const kernel_lut_strided[4] = {
    [MAG_DTYPE_FLOAT32] = &mag_dot_fp8w_scaled_kernel_strided_float,
    [MAG_DTYPE_FLOAT16] = &mag_dot_fp8w_scaled_kernel_strided_mag_float16_t,
    [MAG_DTYPE_BFLOAT16] = &mag_dot_fp8w_scaled_kernel_strided_mag_bfloat16_t,
    [MAG_DTYPE_FLOAT8_E4M3FN] = &mag_dot_fp8w_scaled_kernel_strided_mag_float8_e4m3fn_t
  };
  if (sx == 1 && sy == 1) /* Contig fast path */
    (*kernel_lut_contig[dtype])(N, pr, px, py, scale);
  else
    (*kernel_lut_strided[dtype])(N, pr, px, py, sx, sy, scale);
}

static MAG_HOTPROC void mag_matmul_dot_fp8w_scaled(const mag_kernel_payload_t *payload) {
  mag_tensor_t *r = payload->cmd->out[0];
  const mag_tensor_t *x = payload->cmd->in[0];
  const mag_tensor_t *y = payload->cmd->in[1];
  ...
  int64_t sy = y->coords.strides[0];
  int64_t N = x->coords.shape[0];
  mag_matmul_dot_fp8w_scaled_impl(r->dtype, payload->thread_idx, N, pr, px, sx, py, sy, scale);
}
