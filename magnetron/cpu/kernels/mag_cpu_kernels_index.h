...
#define mag_gen_stub_scatter_body(T, OP) \
    ...
    if (axis < 0) axis += r->coords.rank; \
    int64_t rank = idx->coords.rank; \
    int64_t self_ax = r->coords.shape[axis]; \
    int64_t s_axis = idx->coords.shape[axis]; \
    ...
    if (mag_unlikely(rb <= ra)) return MAG_OK; \
    int64_t ist = idx->coords.strides[axis]; \
    int64_t xst = src->coords.strides[axis]; \
    int64_t rst = r->coords.strides[axis]; \
    int64_t c[MAG_MAX_DIMS]; \
    for (int64_t row=ra; row < rb; ++row) { \
      int64_t rem = row; \
      for (int64_t d = rank-1; d >= 0; --d) { if (d == axis) continue; c[d] = rem % idx->coords.shape[d]; rem /= idx->coords.shape[d]; } \
      int64_t idx_row = 0, src_row = 0, dst_row = 0; \
      for (int64_t d=0; d < rank; ++d) { if (d == axis) continue; idx_row += c[d]*idx->coords.strides[d]; src_row += c[d]*src->coords.strides[d]; dst_row += c[d]*r->coords.strides[d]; } \
