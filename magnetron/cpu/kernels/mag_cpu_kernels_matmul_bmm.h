...
static MAG_HOTPROC void mag_matmul_bmm_fp8w_scaled_dot(const mag_kernel_payload_t *payload) {
  mag_tensor_t *r = payload->cmd->out[0];
  const mag_tensor_t *x = payload->cmd->in[0];
  const mag_tensor_t *y = payload->cmd->in[1];
  ...
  for (int64_t batch=start; batch < end; ++batch) {
    ...
    void *ppr = pr + batch*el;
    const void *ppx = px + mox*el;
    const void *ppy = py + moy;
    mag_matmul_dot_fp8w_scaled_impl(r->dtype, 0, K, ppr, ppx, sx, ppy, sy, scale);
  }
}

static MAG_HOTPROC void mag_matmul_bmm_fp8w_scaled_vec_mat(const mag_kernel_payload_t *payload) {
  mag_tensor_t *r = payload->cmd->out[0];
  const mag_tensor_t *x = payload->cmd->in[0];
  const mag_tensor_t *y = payload->cmd->in[1];
  const mag_tensor_t *s = payload->cmd->in[2];
  ...
  for (int64_t i=start; i < end;) {
    ...
    void *ppr = pr + (batch*N + n0)*el;
    const void *ppx = px + mox*el;
    const void *ppy = py + (moy + n0*sy1);
    mag_matmul_gemv_vec_mat_fp8w_scaled_impl(r->dtype, 0, 1, Nt, K, ppr, ppx, sx, ppy, sy0, sy1, scale);
    i += Nt;
  }
}

static MAG_HOTPROC void mag_matmul_bmm_fp8w_scaled_mat_vec(const mag_kernel_payload_t *payload) {
  mag_tensor_t *r = payload->cmd->out[0];
  const mag_tensor_t *x = payload->cmd->in[0];
  const mag_tensor_t *y = payload->cmd->in[1];
  const mag_tensor_t *s = payload->cmd->in[2];
  ...
  for (int64_t i=start; i < end;) {
    ...
    const void *ppx = px + (mox + m0*sx0)*el;
    const void *ppy = py + moy;
    mag_matmul_gemv_mat_vec_fp8w_scaled_impl(r->dtype, 0,1, Mt, K, ppr, ppx, sx0, sx1, ppy, sy, scale);
    i += Mt;
  }
}

static MAG_HOTPROC void mag_matmul_bmm_fp8w_scaled_gemm(const mag_kernel_payload_t *payload) {
  mag_tensor_t *r = payload->cmd->out[0];
  const mag_tensor_t *x = payload->cmd->in[0];
  const mag_tensor_t *y = payload->cmd->in[1];
  const mag_tensor_t *s = payload->cmd->in[2];
  ...
  for (int64_t i=start; i < end;) {
    ...
    const void *ppx = px + (mox + i0*sx0)*el;
    const void *ppy = py + moy;
    mag_matmul_gemm_fp8w_scaled_impl(r->dtype, 0, 1, Mt, N, K, ppr, ppx, sx0, sx1, ppy, sy0, sy1, scale);
    i += Mt;
  }
}

static MAG_HOTPROC void mag_matmul_bmm_fp8w_scaled(const mag_kernel_payload_t *payload, mag_matmul_type_t type) {
  switch (type) {
    case MAG_MATMUL_TYPE_BMM_DOT: mag_matmul_bmm_fp8w_scaled_dot(payload); return;
    case MAG_MATMUL_TYPE_BMM_GEMV_VEC_MAT: mag_matmul_bmm_fp8w_scaled_vec_mat(payload); return;
    case MAG_MATMUL_TYPE_BMM_GEMV_MAT_VEC: mag_matmul_bmm_fp8w_scaled_mat_vec(payload); return;
    case MAG_MATMUL_TYPE_BMM_GEMM: mag_matmul_bmm_fp8w_scaled_gemm(payload); return;
    default: mag_panic("matmul: invalid BMM matmul type '%s'.", mag_matmul_type_name(type)); return;
  }
}
