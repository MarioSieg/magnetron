...
static MAG_HOTPROC void mag_matmul_bmm_dot(const mag_kernel_payload_t *payload) {
  ...
  int64_t sx = x->meta.coords.strides[xr-1];
  int64_t sy = y->meta.coords.strides[yr-2];
  int64_t bx = xr-2;
  int64_t by = yr-2;
  int64_t br = rr-2;
  ...
  for (int64_t batch=start; batch < end; ++batch) {
    ...
    mag_matmul_dot_impl(r->meta.dtype, 0, K, ppr, ppx, sx, ppy, sy);
  }
}
...
static MAG_HOTPROC void mag_matmul_bmm_vec_mat(const mag_kernel_payload_t *payload) {
  ...
  for (int64_t i=start; i < end;) {
    ...
    mag_matmul_gemv_vec_mat_impl(r->meta.dtype, 0, 1, Nt, K, ppr, ppx, sx, ppy, sy0, sy1);
    i += Nt;
  }
}
...
static MAG_HOTPROC void mag_matmul_bmm_mat_vec(const mag_kernel_payload_t *payload) {
  ...
  int64_t yr = y->meta.coords.rank;
  int64_t rr = r->meta.coords.rank;
  int64_t M = x->meta.coords.shape[xr-2];
  int64_t K = x->meta.coords.shape[xr-1];
  int64_t sx0 = x->meta.coords.strides[xr-2];
  ...
  for (int64_t i=start; i < end;) {
    ...
    const void *ppy = py + moy*el;
    mag_matmul_gemv_mat_vec_impl(r->meta.dtype, 0,1, Mt, K, ppr, ppx, sx0, sx1, ppy, sy);
    i += Mt;
  }
}
