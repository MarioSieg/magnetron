...
typedef void (mag_gemv_vec_mat_fp8w_scaled_kernel_rhs_transposed_contig_t)(int64_t K, int64_t N, void *pr, const void *px, const void *py, float scale);
#define mag_gemv_vec_mat_fp8w_scaled_kernel_rhs_transposed_contig_impl(T, TtoF32, F32toT, LoadTtoF32) \
  static MAG_HOTPROC void mag_gemv_vec_mat_fp8w_scaled_kernel_rhs_transposed_contig_##T(int64_t K, int64_t N, void *pr, const void *px, const void *py, float scale) { \
    T *restrict r = (T *)pr; \
    const T *restrict x = (const T *)px; \
    ...
    for (; j+3 < N; j += 4) { \
      ...
      for (; i+MAG_VF32_LANES-1 < K; i += MAG_VF32_LANES) { \
        mag_vf32_t xv = LoadTtoF32(xp); \
        vacc0 = mag_vf32_fmadd(xv, mag_vf32_loadu_float8_e4m3fn(pw0), vacc0); \
        vacc1 = mag_vf32_fmadd(xv, mag_vf32_loadu_float8_e4m3fn(pw1), vacc1); \
        vacc2 = mag_vf32_fmadd(xv, mag_vf32_loadu_float8_e4m3fn(pw2), vacc2); \
        vacc3 = mag_vf32_fmadd(xv, mag_vf32_loadu_float8_e4m3fn(pw3), vacc3); \
        xp += MAG_VF32_LANES; \
        pw0 += MAG_VF32_LANES; \
        pw1 += MAG_VF32_LANES; \
        pw2 += MAG_VF32_LANES; \
      ...
      for (; i < K; ++i) { \
        float xv = TtoF32(x[i]); \
        sacc0 += xv*mag_float8_e4m3fn_to_float32((y + (j + 0)*K)[i]); \
        sacc1 += xv*mag_float8_e4m3fn_to_float32((y + (j + 1)*K)[i]); \
        sacc2 += xv*mag_float8_e4m3fn_to_float32((y + (j + 2)*K)[i]); \
        sacc3 += xv*mag_float8_e4m3fn_to_float32((y + (j + 3)*K)[i]); \
      } \
      r[j+0] = F32toT(sacc0*scale); \
      r[j+1] = F32toT(sacc1*scale); \
      r[j+2] = F32toT(sacc2*scale); \
      r[j+3] = F32toT(sacc3*scale); \
    } \
    for (; j < N; ++j) { \
      ...
      int64_t i = 0; \
      for (; i+MAG_VF32_LANES-1 < K; i += MAG_VF32_LANES) { \
        acc = mag_vf32_fmadd(LoadTtoF32(xp), mag_vf32_loadu_float8_e4m3fn(wp), acc); \
        xp += MAG_VF32_LANES; \
        wp += MAG_VF32_LANES; \
      } \
      float s = mag_vf32_reduce_add(acc); \
...
mag_gemv_vec_mat_fp8w_scaled_kernel_rhs_transposed_contig_impl(float, mag_cvt_nop, mag_cvt_nop, mag_vf32_loadu)
mag_gemv_vec_mat_fp8w_scaled_kernel_rhs_transposed_contig_impl(mag_float16_t, mag_float16_to_float32, mag_float32_to_float16, mag_vf32_loadu_f16)
mag_gemv_vec_mat_fp8w_scaled_kernel_rhs_transposed_contig_impl(mag_bfloat16_t, mag_bfloat16_to_float32, mag_float32_to_bfloat16, mag_vf32_loadu_bf16)
mag_gemv_vec_mat_fp8w_scaled_kernel_rhs_transposed_contig_impl(mag_float8_e4m3fn_t, mag_float8_e4m3fn_to_float32, mag_float32_to_float8_e4m3fn, mag_vf32_loadu_float8_e4m3fn)
