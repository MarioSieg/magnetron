...
typedef void (mag_gemv_vec_mat_kernel_contig_t)(int64_t K, int64_t N, void *r, const void *px, const void *py);
#define mag_gemv_vec_mat_kernel_contig_impl(dtype, TtoF32, F32toT) \
  static MAG_HOTPROC void mag_gemv_vec_mat_kernel_contig_##dtype(int64_t K, int64_t N, void *pr, const void *px, const void *py) { \
    dtype *r = (dtype *)pr; \
...
static void mag_matmul_gemv_vec_mat(mag_tensor_t *r, const mag_tensor_t *x, const mag_tensor_t *y) {
  ...
  if (sx == 1 && sy0 == N && sy1 == 1) /* Contig fast path */
    (*kernel_lut_contig[r->dtype])(K, N, pr, px, py);
  else
    (*kernel_lut_strided[r->dtype])(K, N, pr, px, py, sx, sy0, sy1);
}
