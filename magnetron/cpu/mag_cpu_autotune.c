...
*/
/*
** Set from an in-model measurement, not from tune_intraop.py. Timing an op on its own from Python
** measures a cold threadpool and operands nobody else has touched; inside a model the pool is hot and
** the operands are already spread across the cores' caches, so threading pays earlier than the tool
** reports. The tool puts this class at 1<<20. At that value every elementwise op on a GPT-2 hidden
** state - 1024x768, or 786K elements, just under the line - runs single-threaded, and prefill at 1024
** tokens costs 1361ms against 1119ms at 1<<18. The autoencoder step does not move either way, since
** nothing in it lands between the two. Re-derive with a real model, not a microbenchmark.
*/
#define MAG_INTRAOP_BANDWIDTH   (1<<18) /* 262144. Measured: ADD, SUB, MUL, DIV, SQR, SQRT, ABS, NEG, RELU, CAST. */
...
#define MAG_INTRAOP_SELECT      (1<<16) /* 65536.   Measured: MASKED_FILL. Trivial arithmetic, but it reads two
                                           operands and writes one, so it scales with cores where FILL's pure
                                           write does not. Same number as EXPENSIVE, different reason. */
/*
** Row-wise ops - the softmax and reduction kernels - split their work by rows, rows = numel/last_dim.
** Over a 1-D tensor that is one row, so only one thread ever has work and threading cannot win at any
** size. Over (rows, cols), which is how a model uses them, they are independent reductions per row and
** scale well: softmax over a 12x1024x1024 attention tensor is 3.5x faster threaded. These are measured
** at 1024 columns, an attention row at GPT-2's context length, not flat.
*/
#define MAG_INTRAOP_ROW_SCAN    (1<<16) /* 65536.   Measured at 1024 wide: SOFTMAX, ARGMIN, ARGMAX. */
...
mag_op_thread_scaling_info mag_cpu_get_op_thread_scaling_info(mag_opcode_t op) {
  static const mag_op_thread_scaling_info scaling_table[MAG_OP__NUM] = {
    ...
    [MAG_OP_MASKED_FILL] = {0.5, MAG_INTRAOP_SELECT},
    ...
    [MAG_OP_MEAN] = {3.5, MAG_INTRAOP_ROW_REDUCE},
    [MAG_OP_MINIMA] = {3.5, MAG_INTRAOP_ROW_MINMAX},
    [MAG_OP_MAXIMA] = {3.5, MAG_INTRAOP_ROW_MINMAX},
    [MAG_OP_MIN] = {3.5, MAG_INTRAOP_ROW_MINMAX},
    [MAG_OP_MAX] = {3.5, MAG_INTRAOP_ROW_MINMAX},
    [MAG_OP_ARGMIN] = {3.5, MAG_INTRAOP_ROW_SCAN},
    [MAG_OP_ARGMAX] = {3.5, MAG_INTRAOP_ROW_SCAN},
    [MAG_OP_SUM] = {3.5, MAG_INTRAOP_ROW_REDUCE},
    [MAG_OP_PROD] = {3.5, MAG_INTRAOP_ROW_REDUCE},
    [MAG_OP_ALL] = {3.5, MAG_INTRAOP_ROW_REDUCE},
    [MAG_OP_ANY] = {3.5, MAG_INTRAOP_ROW_REDUCE},
    ...
    [MAG_OP_SOFTMAX] = {0.9, MAG_INTRAOP_ROW_SCAN},
