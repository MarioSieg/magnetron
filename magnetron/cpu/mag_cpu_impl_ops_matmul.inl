...
static MAG_HOTPROC void mag_mm_block_float32(int64_t kc, int64_t mr, int64_t nr, const float *A, int64_t lda, const float *B, int64_t ldb, float *C, int64_t ldc, bool acc) {
    ...
    for (; nr-j >= 32; j += 32) {
        int64_t i = 0;
        for (; mr-i >= 16; i += 16) mag_mm_tile_16x32_f32(kc, A + i*lda, lda, B + j, ldb, C + i*ldc + j, ldc, acc);
        for (; mr-i >= 8; i += 8) mag_mm_tile_8x32_f32 (kc, A + i*lda, lda, B + j, ldb, C + i*ldc + j, ldc, acc);
        for (; i < mr; ++i) mag_mm_tile_1x32_f32 (kc, A + i*lda, B + j, ldb, C + i*ldc + j, acc);
    }
    for (; nr-j >= 16; j += 16) {
        int64_t i = 0;
        for (; mr-i >= 8; i += 8) mag_mm_tile_8x16_f32 (kc, A + i*lda, lda, B + j, ldb, C + i*ldc + j, ldc, acc);
        for (; i < mr; ++i) mag_mm_tile_1x16_f32 (kc, A + i*lda, B + j, ldb, C + i*ldc + j, acc);
    }
    for (; nr-j >= 8; j += 8) {
        int64_t i = 0;
        for (; mr-i >= 8; i += 8) mag_mm_tile_8x8_f32 (kc, A + i*lda, lda, B + j, ldb, C + i*ldc + j, ldc, acc);
        for (; i < mr; ++i) mag_mm_tile_1x8_f32 (kc, A + i*lda, B + j, ldb, C + i*ldc + j, acc);
    }
    int64_t rem = nr-j;
    if (!rem) return;
...
MAG_HOTPROC static void mag_matmul_float32(const mag_kernel_payload_t *payload) {
    ...
    for (;;) {
        ...
        for (int64_t pc = 0; pc < K; pc += KC) {
            ...
            if (y->coords.rank == 1) mag_mm_pack_B_vec_f32(kc, nc, py_base + pc, Bp);
            else mag_mm_pack_B_kc_nc_f32(kc, nc, py_base + pc*sKy +  j0*sNy, sKy, sNy, Bp);
