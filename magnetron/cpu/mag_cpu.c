...
static const mag_op_thread_scaling_info mag_op_thread_scalings[MAG_OP__NUM] = {
...
static void mag_mm_autotune_block_params(const mag_matmul_block_tune_info_t *info, mag_matmul_block_params_t *params) {
...
static uint32_t mag_cpu_tune_heuristics_intraop_workers(const mag_command_t *cmd, mag_device_t *dvc) {
...
static uint32_t mag_cpu_dynamic_work_scaling(mag_cpu_device_t *dvc, mag_opcode_t op, int64_t numel) {
    ...
    const mag_op_thread_scaling_info *info = mag_op_thread_scalings+op;
