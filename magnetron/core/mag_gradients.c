...
mag_status_t mag_op_backward_mean(mag_au_state_t *node, mag_tensor_t **grads) {
    ...
    mag_tensor_t *x = node->op_inputs[0];
    mag_tensor_t *scale;
    stat = mag_tensor_full_like(&scale, x, mag_scalar_float(1.0 / (double)x->numel));
...
mag_status_t mag_op_backward_sum(mag_au_state_t *node, mag_tensor_t **grads) {
    ...
    mag_tensor_t *x = node->op_inputs[0];
    mag_tensor_t *ones;
    stat = mag_tensor_full_like(&ones, x, mag_scalar_float(1.0));
    if (mag_iserr(stat)) return stat;
...
mag_status_t mag_op_backward_abs(mag_au_state_t *node, mag_tensor_t **grads) {
    ...
    if (mag_iserr(stat)) return stat;
    stat = mag_tensor_scalar(&one, x->ctx, x->dtype, mag_scalar_float(1.0));
    if (mag_iserr(stat)) goto error;
    stat = mag_tensor_scalar(&two, x->ctx, x->dtype, mag_scalar_float(2.0));
    if (mag_iserr(stat)) goto error;
...
mag_status_t mag_op_backward_neg(mag_au_state_t *node, mag_tensor_t **grads) {
    ...
    mag_status_t stat = mag_tensor_scalar(&m1, node->grad->ctx, node->grad->dtype, mag_scalar_float(-1.0));
    if (mag_iserr(stat)) return stat;
...
mag_status_t mag_op_backward_sqr(mag_au_state_t *node, mag_tensor_t **grads) {
    mag_tensor_t *x = node->op_inputs[0];
    mag_tensor_t *two;
    mag_status_t stat = mag_tensor_scalar(&two, x->ctx, x->dtype, mag_scalar_float(2.0));
    if (mag_iserr(stat)) return stat;
    mag_tensor_t *two_x;
