...
void mag_mmap_ra_register(const mag_mapped_file_t *mf) {
  ...
  if (!window_mb || !mf->map || !mf->fs) return;
  ...
  ra->page = pg > 0 ? (uint64_t)pg : 4096;
  int rel = mag_envcfg_readahead_release();
  if (rel < 0) {
    long phys_pages = sysconf(_SC_PHYS_PAGES);
    uint64_t phys = phys_pages > 0 ? (uint64_t)phys_pages*ra->page : 0;
    rel = phys && ra->fs > phys/4*3;
  }
  ra->release = rel > 0;
  ...
  mag_error_t err = {0};
  if (!ra->log || !ra->hash_key || !ra->hash_idx || !ra->queue || !ra->scratch || mag_mutex_create(&err, &ra->mtx) != MAG_OK || mag_condvar_create(&err, &ra->cv) != MAG_OK) {
    mag_ra_free(ra);
    return;
  }
...
static void mag_ra_hint(mag_ra_engine_t *ra, uint64_t off, uint64_t len) {
  ...
  uint64_t key = off+1;
  mag_mutex_lock(&err, ra->mtx);
  if (!ra->thread && mag_thread_create(&err, &ra->thread, &mag_ra_worker, MAG_THREAD_PRIO_NORMAL, "mag_readahead", ra) != MAG_OK) {
    ra->thread = NULL;
    mag_mutex_unlock(&err, ra->mtx);
    return;
  }
...
void mag_mmap_readahead_hint(const void *ptr, size_t len) {
  if (len < MAG_RA_MIN_HINT_BYTES) return;
