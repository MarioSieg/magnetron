...
mag_status_t mag_pgroup_init_tcp(
  ...
  if (mag_unlikely(out == NULL))
    return mag_set_error(err, MAG_ERR_PARAM, "process_group: out ptr is NULL.");
  *out = NULL;
  if (mag_unlikely(master_addr == NULL || !*master_addr))
    return mag_set_error(err, MAG_ERR_PARAM, "process_group: master_addr is invalid.");
  if (mag_unlikely(world_size == 0))
    return mag_set_error(err, MAG_ERR_PARAM, "process_group: world_size (%u) must be > 0.", world_size);
  if (mag_unlikely(rank >= world_size))
    return mag_set_error(err, MAG_ERR_PARAM, "process_group: rank (%u) must be in [0, %u).", rank, world_size);

  mag_tcp_socket_t **peers = (*mag_try_alloc)(NULL, sizeof(*peers)*world_size, 0);
  if (mag_unlikely(peers == NULL))
    return mag_set_error(err, MAG_ERR_OOM, "process_group: failed to allocate %zu bytes.", sizeof(*peers)*world_size);
  ...
  if (mag_unlikely(pgroup == NULL)) {
    (*mag_alloc)(peers, 0, 0);
    return mag_set_error(err, MAG_ERR_OOM, "process_group: failed to allocate %zu bytes.", sizeof(*pgroup));
  }
  *pgroup = (mag_process_group_t){.rank=rank, .world_size=world_size, .peers=peers};

  if (world_size == 1) { *out = pgroup; return MAG_OK; }
  ...
  if (rank == 0) { /* Master node */
    if (mag_unlikely(!mag_tcp_socket_listen(&listener, master_port, 128))) {
      status = mag_set_error(err, MAG_ERR_IO, "process_group: rank 0 failed to listen on port %u.", master_port);
      goto cleanup;
    }
    for (uint32_t ra = 1; ra < world_size; ++ra) {
      mag_tcp_socket_t *sock = NULL;
      if (mag_unlikely(!mag_tcp_socket_accept(&sock, listener))) {
        status = mag_set_error(err, MAG_ERR_IO, "process_group: rank 0 failed to accept peer.");
        goto cleanup;
      }
      ...
      if (mag_unlikely(!mag_tcp_socket_recv_all(sock, &peer_rank, sizeof(peer_rank)))) {
        ...
        status = mag_set_error(err, MAG_ERR_IO, "process_group: rank 0 failed to receive peer rank.");
        goto cleanup;
      }
      if (mag_unlikely(!(peer_rank > 0 && peer_rank < world_size && peers[peer_rank] == NULL))) {
        mag_tcp_socket_close(sock);
        status = mag_set_error(err, MAG_ERR_PARAM, "process_group: received invalid or duplicate peer rank %u (expected a unique rank in [1, %u)).", peer_rank, world_size);
        goto cleanup;
      }
    ...
    if (mag_unlikely(!mag_tcp_socket_connect(&sock, master_addr, master_port))) {
      status = mag_set_error(err, MAG_ERR_IO, "process_group: rank %u failed to connect to %s:%u.", rank, master_addr, master_port);
      goto cleanup;
    }
    if (mag_unlikely(!mag_tcp_socket_send_all(sock, &rank, sizeof(rank)))) {
      mag_tcp_socket_close(sock);
      status = mag_set_error(err, MAG_ERR_IO, "process_group: rank %u failed to send rank id.", rank);
      goto cleanup;
    }
    *peers = sock;
  }
...
mag_status_t mag_pgroup_validate(mag_error_t *err, mag_process_group_t *pgroup) {
  if (mag_unlikely(pgroup == NULL))
    return mag_set_error(err, MAG_ERR_PARAM, "process_group: group ptr is NULL.");
  if (mag_unlikely(!(pgroup->rank < pgroup->world_size)))
    return mag_set_error(err, MAG_ERR_PARAM, "process_group: rank (%u) must be in [0, %u).", pgroup->rank, pgroup->world_size);
  if (mag_unlikely(!(pgroup->world_size > 0)))
    return mag_set_error(err, MAG_ERR_PARAM, "process_group: world_size (%u) must be > 0.", pgroup->world_size);
  return MAG_OK;
}

mag_status_t mag_pgroup_verify_tensor_is_wireable(mag_error_t *err, mag_tensor_t *tensor) {
  if (mag_unlikely(tensor == NULL))
    return mag_set_error(err, MAG_ERR_PARAM, "process_group: tensor ptr is NULL.");
  if (mag_unlikely(!mag_tensor_is_cpu(tensor)))
    return mag_set_error(err, MAG_ERR_PARAM, "process_group: tensor must be on CPU to be wireable.");
  if (mag_unlikely(!mag_tensor_is_contiguous(tensor)))
    return mag_set_error(err, MAG_ERR_PARAM, "process_group: tensor must be contiguous to be wireable.");
  if (mag_unlikely(!mag_tensor_is_numeric_typed(tensor)))
    return mag_set_error(err, MAG_ERR_PARAM, "process_group: tensor must be numeric typed to be wireable.");
  return MAG_OK;
}
...
mag_status_t mag_pgroup_send_bytes(mag_error_t *err, mag_process_group_t *pgroup, uint32_t dst_rank, const void *buf, size_t nb) {
  ...
  if (mag_unlikely(!(dst_rank < pgroup->world_size)))
    return mag_set_error(err, MAG_ERR_PARAM, "process_group: dst rank (%u) must be in [0, %u).", dst_rank, pgroup->world_size);
  if (mag_unlikely(dst_rank == pgroup->rank))
    return mag_set_error(err, MAG_ERR_PARAM, "process_group: dst rank (%u) must != current process group rank (%u).", dst_rank, pgroup->rank);
  if (mag_unlikely(pgroup->peers[dst_rank] == NULL))
    return mag_set_error(err, MAG_ERR_PARAM, "process_group: dst rank (%u) socket is invalid.", dst_rank);
  if (mag_unlikely(!mag_tcp_socket_send_all(pgroup->peers[dst_rank], buf, nb)))
    return mag_set_error(err, MAG_ERR_IO, "process_group: failed to send buffer of size %zuB to dst rank (%u)", nb, dst_rank);
  return MAG_OK;
}
...
mag_status_t mag_pgroup_recv_bytes(mag_error_t *err, mag_process_group_t *pgroup, uint32_t src_rank, void *buf, size_t nb) {
  ...
  if (mag_unlikely(!(src_rank < pgroup->world_size)))
    return mag_set_error(err, MAG_ERR_PARAM, "process_group: src rank (%u) must be in [0, %u).", src_rank, pgroup->world_size);
  if (mag_unlikely(src_rank == pgroup->rank))
    return mag_set_error(err, MAG_ERR_PARAM, "process_group: src rank (%u) must != current process group rank (%u).", src_rank, pgroup->rank);
  if (mag_unlikely(pgroup->peers[src_rank] == NULL))
    return mag_set_error(err, MAG_ERR_PARAM, "process_group: src rank (%u) socket is invalid.", src_rank);
  if (mag_unlikely(!mag_tcp_socket_recv_all(pgroup->peers[src_rank], buf, nb)))
    return mag_set_error(err, MAG_ERR_IO, "process_group: failed to recv buffer of size %zuB from src rank (%u)", nb, src_rank);
  return MAG_OK;
}
...
mag_status_t mag_pgroup_barrier(mag_error_t *err, mag_process_group_t *pgroup) {
  mag_status_t status = mag_pgroup_validate(err, pgroup);
  if (mag_iserr(status)) return status;
  if (pgroup->world_size == 1) return MAG_OK;
  uint8_t token=1;
