...
void mag_snap_hdr_encode(uint8_t *p, const mag_snap_decoded_hdr_t *hdr) {
  ...
  mag_snap_wu16(&delta, hdr->version);
  mag_snap_wu16(&delta, hdr->aux);
...
static bool mag_snap_write_zero_pad(mag_snapshot_stream_writer_t *w, size_t nb) {
  ...
  while (nb) {
    size_t chunk = mag_xmin(nb, sizeof(zero_chunk));
...
mag_status_t mag_snapshot_stream_writer_open(
  ...
  *writer = NULL;
  ...
  mag_snap_decoded_hdr_t hdr = {
    ...
    .aux = 0, /* Unused for now must be 0 */
    .meta_range = {
      .offset = MAG_SNAP_HDR_SIZE,
      .span = meta_len
    },
    .blob_range = {
      .offset = mag_snap_alignup(MAG_SNAP_HDR_SIZE+meta_len, MAG_SNAP_FILE_BLOB_ALIGN),
      .span = blob_len
    }
  };
...
mag_status_t mag_snapshot_stream_writer_submit_blob(
  ...
  if (mag_unlikely(size > writer->blob_len-writer->nb_emitted))
    return mag_set_error(err, MAG_ERR_PARAM, "snapshot: '%s' received %" PRIu64 " bytes more than the %" PRIu64 " it declared.", writer->path, size-(writer->blob_len-writer->nb_emitted), writer->blob_len);
...
mag_status_t mag_snapshot_stream_writer_close(mag_error_t *err, mag_snapshot_stream_writer_t *writer) {
  ...
  if (mag_unlikely(fflush(writer->F) != 0)) {
    st = mag_set_error(err, MAG_ERR_IO, "snapshot: failed to flush '%s'.", writer->tmp_path);
    goto fail;
  }
  fclose(writer->F);
  writer->F = NULL;
  ...
  if (mag_unlikely(rename(writer->tmp_path, writer->path) != 0)) {
    st = mag_set_error(err, MAG_ERR_IO, "snapshot: failed to rename '%s' to '%s'.", writer->tmp_path, writer->path);
    goto fail;
  }
...
void mag_snapshot_stream_writer_abort(mag_snapshot_stream_writer_t *writer) {
  ...
  if (!writer->is_closed && writer->tmp_path)
