...
mag_status_t mag_tensor_grad(mag_error_t *err, const mag_tensor_t *tensor, mag_tensor_t **out_grad) {
  mag_contract(err, ERR_INVALID_PARAM, {}, tensor->flags & MAG_TFLAG_REQUIRES_GRAD, "autograd: tensor does not require gradient; enable requires_grad to access its gradient.");
  mag_contract(err, ERR_INVALID_STATE, {}, tensor->au_state, "autograd: autodiff state is missing for this tensor.");
...
mag_status_t mag_tensor_set_requires_grad(mag_error_t *err, mag_tensor_t *tensor, bool requires_grad) {
  if (requires_grad) {
    mag_contract(err, ERR_INVALID_PARAM, {}, mag_tensor_is_floating_point_typed(tensor), "autograd: gradient tracking requires a floating-point dtype, but tensor has dtype %s.", mag_type_trait(tensor->dtype)->name);
...
mag_status_t mag_tensor_backward(mag_error_t *err, mag_tensor_t *root) {
  ...
  mag_contract(err, ERR_INVALID_PARAM, {}, root->flags & MAG_TFLAG_REQUIRES_GRAD, "autograd: cannot backpropagate from a tensor that does not require gradients; create it with requires_grad=True.");
  mag_contract(err, ERR_INVALID_PARAM, {}, root->coords.rank == 0 && root->numel == 1, "autograd: backpropagation requires a scalar root tensor (rank 0, numel 1).");
  ...
  for (size_t id=0; id < post_order.size; ++id) {
    ...
    mag_contract(err, ERR_INVALID_STATE, { stat = MAG_STATUS_ERR_INVALID_STATE; goto cleanup; }, child && child->au_state, "autograd: autodiff state is missing for a tensor in the computation graph.");
    ...
    mag_contract(err, ERR_INVALID_STATE, { stat = MAG_STATUS_ERR_INVALID_STATE; goto cleanup; }, backward != NULL, "autograd: operator '%s' has no backward implementation.", meta->mnemonic);
