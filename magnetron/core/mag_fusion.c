...
static void mag_fuse_codegen_ss(const mag_fuse_plan_t *plan, mag_sstream_t *ss) {
  mag_sstream_append(ss,
    ...
    "#include <math.h>\n"
    "#include <stdint.h>\n\n"
    "void mag_fused(void *const *bufs, const double *imms, int64_t begin, int64_t end) {\n");
  for (uint8_t b=0; b < plan->num_bufs; ++b) {
    ...
    if (written) mag_sstream_append(ss, "  float *b%u = (float *)bufs[%u];\n", b, b);
    else mag_sstream_append(ss, "  const float *restrict b%u = (const float *)bufs[%u];\n", b, b);
  }
  for (uint8_t k=0; k < plan->num_imms; ++k)
    mag_sstream_append(ss, "  const float s%u = (float)imms[%u];\n", k, k);
  ...
  for (uint32_t k=0; k < plan->num_ins; ++k) {
    ...
    if (ins->op == MAG_OP_NOP) { /* a plain load of buffer element i */
      mag_sstream_append(ss, "    const float r%u = %s;\n", ins->dst, names[0]);
      continue;
    }
    const mag_fuse_op_form_t *form = mag_fuse_form((mag_opcode_t)ins->op);
    mag_sstream_append(ss, "    const float r%u = ", ins->dst);
...
static uint64_t mag_fuse_plan_hash(const mag_fuse_plan_t *plan) {
  /* Only the structure is hashed. num_imms is included but immediate values are not, so a new
     learning rate reuses the kernel. */
...
mag_status_t mag_fuse_compile(mag_error_t *err, mag_context_t *ctx, const mag_fuse_plan_t *plan, mag_fused_fn_t *out_fn) {
  *out_fn = NULL;
  if (mag_unlikely(plan->dtype != MAG_DTYPE_FLOAT32))
    return mag_set_error(err, MAG_ERR_OP, "fusion: only float32 chains are supported.");
...
static bool mag_fuse_build_adam(mag_fuse_plan_t *plan) {
...
bool mag_fused_adam_supported(const mag_tensor_t *p, const mag_tensor_t *g, const mag_tensor_t *m, const mag_tensor_t *v) {
  ...
  for (int i=0; i < 4; ++i) {
    ...
    if (all[i]->meta.dtype != MAG_DTYPE_FLOAT32) return false;
