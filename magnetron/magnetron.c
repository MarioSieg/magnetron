...
void *(*mag_get_alloc_fn(void))(void *, size_t, size_t) {
    return mag_alloc;    /* Get global allocator. */
}
void mag_set_alloc_fn(void *(*alloc)(void *, size_t, size_t)) {
    mag_assert2(alloc);    /* Set global allocator. */
    mag_alloc = alloc;
}
...
uint32_t mag_pack_color_u8(uint8_t r, uint8_t g, uint8_t b) {
...
mag_tensor_t *mag_tensor_empty_scalar(mag_context_t *ctx, mag_dtype_t type) {
    return mag_tensor_empty(ctx, type, 1, (int64_t[1]) {
        1
    });
}
