...
TEST(romap, put_get_roundtrip) {
    ...
    EXPECT_TRUE(mag_map_insert_if_absent(&m, "hello", 5, v1));
    EXPECT_EQ(mag_map_lookup(&m, "hello", 5), v1);
    EXPECT_EQ(mag_map_lookup(&m, "hella", 5), nullptr);

    EXPECT_TRUE(mag_map_insert_if_absent(&m, "world", 5, v2));
...
TEST(romap, put_duplicate_key_returns_old_value_and_keeps_old) {
    ...
    EXPECT_TRUE(mag_map_insert_if_absent(&m, "k", 1, v1));
    EXPECT_EQ(mag_map_lookup(&m, "k", 1), v1);

    EXPECT_TRUE(mag_map_insert_if_absent(&m, "k", 1, v2)); /* Already present: keeps old value v1. */
    EXPECT_EQ(mag_map_lookup(&m, "k", 1), v1);
...
TEST(romap, delete_existing_and_missing) {
    ...
    EXPECT_TRUE(mag_map_insert_if_absent(&m, "a", 1, v1));
...
TEST(romap, many_inserts_trigger_resize_and_all_findable) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        EXPECT_TRUE(mag_map_insert_if_absent(&m, keys.back().data(), keys.back().size(), v));
    }
...
TEST(romap, clone_keys_true_key_bytes_stable) {
    ...
    EXPECT_TRUE(mag_map_insert_if_absent(&m, buf, 8, v));
