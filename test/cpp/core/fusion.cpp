...
TEST(fusion, classifies_fusible_ops) {
    ...
    ASSERT_TRUE(mag_fuse_op_is_pointwise(MAG_OP_RELU));
    /* Approximated in the CPU vector kernels, so generated libm code would not reproduce them. */
    ASSERT_FALSE(mag_fuse_op_is_pointwise(MAG_OP_GELU));
    ASSERT_FALSE(mag_fuse_op_is_pointwise(MAG_OP_TANH));
    ASSERT_FALSE(mag_fuse_op_is_pointwise(MAG_OP_EXP));
...
TEST(fusion, compiled_adam_matches_reference) {
    ...
    mag_fuse_plan_t plan;
    mag_fuse_plan_init(&plan, MAG_DTYPE_FLOAT32);
    /* bufs: 0=param 1=grad 2=m 3=v   imms: 0=b1 1=b2 2=lr 3=eps 4=c1 5=c2 6=1.0
       1.0 is an immediate rather than a literal so that (1-beta) is computed inside the
       kernel, at the same width and in the same order as the eager chain computes it. */
    int32_t p = mag_fuse_load(&plan, 0);
    int32_t g = mag_fuse_load(&plan, 1);
    ...
    /* Built with -ffp-contract=off precisely so fusing a chain cannot change its arithmetic:
       the generated kernel must agree with the op-by-op reference bit for bit. ASSERT_FLOAT_EQ
       allows four ULP, which would not test that claim. */
    for (int64_t i=0; i < n; ++i) {
        ASSERT_EQ(bits(p_j[i]), bits(p_r[i])) << "param mismatch at " << i;
        ASSERT_EQ(bits(m_j[i]), bits(m_r[i])) << "moment mismatch at " << i;
        ASSERT_EQ(bits(v_j[i]), bits(v_r[i])) << "variance mismatch at " << i;
    }
}
