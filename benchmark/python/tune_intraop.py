...
Shape matters as much as size for anything that reduces. The softmax and reduction kernels split
their work by rows - rows = numel / last_dim - so over a 1-D tensor there is exactly one row, only
one thread can ever have work, and threading cannot win at any size. Over (rows, cols), which is
how a model uses them, they are rows independent reductions and parallelise freely. Measuring those
ops flat says 'never threads' about the one shape nobody runs, so they are measured at ROW_COLS
wide instead.
...
ROW_COLS = 1024  # Width to reduce over for row-wise ops: an attention row at GPT-2's context.
...
OPS = {
    ...
    'SOFTMAX': ('rowwise', lambda a: a.softmax(dim=-1)),
    ...
    'MASKED_FILL': ('masked', lambda a, m: a.masked_fill(m, 0.0)),
    'CAST': ('unary', lambda a: a.cast(dtype.float16)),
    'CLONE': ('unary', lambda a: a.clone()),
    'SUM': ('rowwise', lambda a: a.sum(-1)),
    'MEAN': ('rowwise', lambda a: a.mean(-1)),
    'MAX': ('rowwise', lambda a: a.max(-1)),
    'ARGMAX': ('rowwise', lambda a: a.argmax(-1)),
}
...
def _measure(op: str, size: int, repeats: int) -> int:
    ...
    if kind == 'rowwise':
        # Reduced over the last dimension, so the kernel has size // ROW_COLS rows to spread.
        r = Tensor.uniform((max(1, size // ROW_COLS), ROW_COLS), low=1.0, high=2.0)
        return _time_calls(lambda: fn(r), repeats)
    if kind == 'masked':
        # A same-shape mask, which is the run-walking path. The general broadcast path costs more
        # per element, so a threshold measured here threads no later than one measured there.
        m = Tensor.uniform((size,), low=0.0, high=1.0) < 0.5
        return _time_calls(lambda: fn(a, m), repeats)
