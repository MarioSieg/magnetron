...
| `unbind(d)`                               | Remove dim, return tuple of views                                                                                           | N/A                                        | `x.unbind(0)`                   |
...
Inputs are batched: `[N, C, L]`, `[N, C, H, W]` or `[N, C, D, H, W]`. Weight is `[C_out, C_in/groups, *K]` for `convXD` and `[C_in, C_out/groups, *K]` for `convTXD`. Also available as layers `nn.Conv1D`, `nn.Conv2D`, `nn.Conv3D`, `nn.ConvT1D`, `nn.ConvT2D`, `nn.ConvT3D`, `nn.Upsample` and `nn.GroupNorm`.
...
| `interpolate(size=None, scale_factor=None, mode='nearest', align_corners=False, antialias=False)` | Resample the spatial dims. Modes: `nearest`, `nearest-exact`, `linear` (3D), `bilinear`/`bicubic` (4D), `trilinear` (5D), `area`. `antialias` applies to bilinear and bicubic. | $y_{n,c,j}=\sum_i w_{j,i}\,x_{n,c,i}$                                   | `y = x.interpolate(scale_factor=2, mode='bilinear')` |
