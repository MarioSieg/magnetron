...
| `Tensor.one_hot(c)`            | Convert indices to one-hot vectors                                        | $y_{i,j}=[x_i=j]$             | `x = i.one_hot(10)`                              |
...
| `topk(k)`  | Select k largest values           | $ \mathrm{topk}(x,k)=\{(x_{i_j},i_j)\}_{j=1}^k,\;x_{i_1}\ge\dots\ge x_{i_k} $ | `values, indices = x.topk(k)` |
